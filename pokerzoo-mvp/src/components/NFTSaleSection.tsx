      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-12">
        ...
        <div className="flex-1">
          <div className="bg-white/5 backdrop-blur-md p-8 rounded-xl border border-yellow-400/30 shadow-lg space-y-5">
            ...
            <Field label="Game" name="game" value={form.game} onChange={handleChange} />
            <Field label="Ticket Price ($)" name="price" value={form.price} onChange={handleChange} />
            <Field label="Entries" name="type" value={form.type} onChange={handleChange} />
            <Field label="Date" name="date" value={form.date} onChange={handleChange} />
            <Field label="Supply" name="supply" value={form.supply} onChange={handleChange} />
            <Field label="Refund %" name="refund" value={form.refund} onChange={handleChange} />

            {/* Color pickers */}
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <ColorField label="Border 1" name="borderColor" value={form.borderColor} onChange={handleChange} />
              <ColorField label="Border 2" name="imgBorderColor" value={form.imgBorderColor} onChange={handleChange} />
              <ColorField label="Background" name="bgColor" value={form.bgColor} onChange={handleChange} />
              <ColorField label="Features" name="featuresColor" value={form.featuresColor} onChange={handleChange} />
              <ColorField label="Text" name="textColor" value={form.textColor} onChange={handleChange} />
            </div>

            {/* Background style */}
            <label className="flex flex-col gap-1 text-sm">
              <span className="font-medium text-yellow-300">Background Style</span>
              <select name="bgStyle" value={form.bgStyle} onChange={handleChange} className="px-3 py-2 rounded-md text-[#0c1a3a]">
...
const Field = ({ label, name, value, onChange, textarea }: FieldProps) => (
  <label className="flex flex-col gap-1 text-sm">
    ...
    {textarea ? (
      <textarea name={name} value={value} onChange={onChange} rows={3} className="px-3 py-2 rounded-md text-[#0c1a3a]" />
    ) : (
      <input type="text" name={name} value={value} onChange={onChange} className="px-3 py-2 rounded-md text-[#0c1a3a]" />
    )}
  </label>
);

const ColorField = ({ label, name, value, onChange }: { label: string; name: string; value: string; onChange: (e: ChangeEvent<HTMLInputElement>) => void }) => (
  <label className="flex flex-col gap-1 text-sm items-start">
    <span className="font-medium text-yellow-300">{label}</span>
    <input type="color" name={name} value={value} onChange={onChange} className="w-10 h-8 border-none cursor-pointer" />
  </label>
);
...
const SvgPreview = (p: Record<string, string>) => (
  ...
  >
    ...
    {p.bgStyle === 'crosses' && (
      <defs>
        <pattern id="cross" width="16" height="16" patternUnits="userSpaceOnUse">
          <path d="M0 5 H10 M5 0 V10" stroke={p.featuresColor} strokeWidth="2"/>
        </pattern>
      </defs>
    )}
    {p.bgStyle === 'hearts' && (
      <defs>
        <pattern id="hearts" width="28" height="28" patternUnits="userSpaceOnUse">
          <path d="M14 25 L5 16 C1 12 1 6 5 4 C8 3 11 4 14 7 C17 4 20 3 23 4 C27 6 27 12 23 16 L14 25 Z" fill={p.featuresColor} />
        </pattern>
      </defs>
    )}
    {p.bgStyle === 'dots' && <rect width="520" height="720" fill="url(#dots)" />}
    {p.bgStyle === 'crosses' && <rect width="520" height="720" fill="url(#cross)" />}
    {p.bgStyle === 'hearts' && <rect width="520" height="720" fill="url(#hearts)" />}
    ...
    {/* details header + bg */}
    <rect x="40" y="425" width="440" height="264" rx="12" fill="#FFFFFF" opacity="0.1" />
    <text
      x="30%"
      y="570"
      fill={p.textColor}
